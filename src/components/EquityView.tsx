...
export default function EquityView({ profiles, factors, currency }: Props) {
  ...
  if (profiles.length < 2) {
    return (
      <div className="max-w-2xl mx-auto">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">{t('equity.title')}</h1>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">{t('equity.title')}</h1>
        ...
        <div className="space-y-2">
          {sorted.map(({ id, name, salary }) => {
            const delta = medianSalary > 0
              ...
              : 0
            const deltaColor =
              delta > 0 ? 'text-green-700 bg-green-50' : delta < 0 ? 'text-red-600 bg-red-50' : 'text-gray-500 bg-gray-100'
            const deltaLabel = delta > 0 ? `+${delta}%` : `${delta}%`
            return (
              <div key={id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                <span className="font-medium text-gray-800 truncate max-w-[50%]">{name}</span>
