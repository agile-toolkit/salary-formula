...
| `salary-formula:lastSession` | `{lastScenario: string\|null, profileCount: number, totalSalaryRange: {min, max, currency, mixedCurrencies: boolean}\|null, updatedAt: ISO string}` | Dashboard card summary; written on every profile/scenario save. `min`/`max` are labeled with each profile's own saved currency where available; `mixedCurrencies` flags when profiles span more than one currency so `currency` alone shouldn't be trusted as the label for both ends of the range. |
...
| `sprint_metrics_salary_bridge_v1` | `{profiles: [{name, annualSalary, currency}], exportedAt}` | Read by Sprint Metrics for team cost/sprint math; written on demand via "Share with Sprint Metrics". Each profile's `currency` is its own saved currency, not the current selector. |
